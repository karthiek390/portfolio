...
export default function BlueContactForm21st() {
  ...
  const trackFirstInteraction = () => {
    ...
    trackOperatorEvent({
      type: "CONTACT_INIT",
      detail: "blue-contact-first-focus",
      page: "blue",
      metadata: { mode: "blue" },
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      trackOperatorEvent({
        type: "CONTACT_SENT",
        detail: "blue-contact-submitted",
        page: "blue",
        metadata: { mode: "blue", company: form.company.trim() },
      });
    } catch {
      setStatus("error");
  ...
  const Field = ({
    ...
    <label style={{ display: "grid", gap: "0.5rem" }}>
      <span style={{ color: "#0F172A", fontSize: "0.9rem", fontWeight: 600 }}>{label}</span>
      {multiline ? (
        <textarea
          value={form[k]}
