...
export default function RedSkillsSection() {
  ...
  const displayId = hovered ?? activeId;
  const displayNode = displayId ? nodeById(displayId) : null;
  const displayNodes = clicked.length > 0 ? clicked.map((id) => nodeById(id)) : displayNode ? [displayNode] : [];
  const mutation = activeId ? MUTATIONS[activeId] : null;
  ...
  const handleNodeClick = useCallback((id: string) => {
    setClicked((prev) => {
      ...
      const next = exists ? prev.filter((nodeId) => nodeId !== id) : [...prev, id];

      if (exists) {
        setActiveId((current) => {
          if (current !== id) return current;
          const fallback = next[next.length - 1] ?? null;
          const fallbackMutation = fallback ? MUTATIONS[fallback] : null;
          setServerFlavour(fallback ? fallbackMutation?.flavour ?? null : null);
          setServerAccent(fallback ? fallbackMutation?.accent ?? null : null);
          setServerQuote(fallback ? ORACLE_QUOTE : null);
          return fallback;
        });
        return next;
      }
  ...
  const handleSkillClick = useCallback((nodeId: string, skill: string) => {
    setMutatedSkill(`${nodeId}:${skill}`);
    setTimeout(() => setMutatedSkill(null), 1200);
    postSkillClick(nodeId, skill);
  }, [postSkillClick]);
  ...
  return (
    ...
    >
      ...
      <motion.h2
        animate={{ color: sectionAccent, textShadow: `0 0 12px ${sectionAccent}55` }}
      ...
      <p style={{ color: "#00802B", fontSize: "0.82rem", marginBottom: "3rem", fontFamily: mono }}>
        {clicked.length > 0
          ? "Click nodes to select or deselect multiple programs. Click a skill to mutate."
          : "Hover to inspect. Click nodes to load multiple programs."}
      </p>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
          ...
          <div style={{ position: "absolute", inset: 0 }}>
            ...
            >
              {EDGES.map(([a, b]) => {
                const na = nodeById(a);
                const nb = nodeById(b);
                const isClickedEdge = clicked.includes(a) || clicked.includes(b);
        ...
        <motion.div
          animate={{ borderColor: `${sectionAccent}44` }}
          transition={{ duration: 0.4 }}
          style={{
            minHeight: "120px",
        ...
        >
          <AnimatePresence mode="wait">
            {displayNodes.length > 0 ? (
              <motion.div
                key={displayNodes.map((node) => node.id).join("-")}
              ...
              >
                <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                  {displayNodes.map((node) => {
                    const nodeAccent = MUTATIONS[node.id]?.accent ?? sectionAccent;
                    const isSelected = clicked.includes(node.id);
                    return (
                      <div key={node.id}>
                        <p style={{ color: "#003B00", fontSize: "0.6rem", letterSpacing: "0.15em", marginBottom: "0.75rem" }}>
                          [{node.label}] // {node.items.length} MODULES LOADED
                          {isSelected && " // ORACLE_ACTIVE"}
                        </p>
                        <div style={{ display: "flex", flexWrap: "wrap", gap: "0.4rem" }}>
                          {node.items.map((skill) => {
                            const isMutated = mutatedSkill === `${node.id}:${skill}`;
                            return (
                              <motion.span
                                key={`${node.id}-${skill}`}
                                onClick={() => isSelected ? handleSkillClick(node.id, skill) : undefined}
                                initial={{ opacity: 0, scale: 0.9 }}
                                animate={{
                                  opacity: 1,
                                  scale: isMutated ? 1.12 : 1,
                                  backgroundColor: isMutated ? nodeAccent : "transparent",
                                  color: isMutated ? "#000" : nodeAccent,
                                }}
                                transition={{ duration: 0.12 }}
                                style={{
                                  padding: "0.2rem 0.6rem",
                                  border: `1px solid ${nodeAccent}55`,
                                  borderRadius: "2px",
                                  fontSize: "0.75rem",
                ...
                {activeId && clicked.includes(activeId) && MUTATIONS[activeId] && (
                  <motion.p
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                  ...
                  >
                    &gt; {serverFlavour ?? MUTATIONS[activeId].flavour}
                  </motion.p>
                )}
              </motion.div>
            ) : (
              <motion.p
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                style={{ color: "#00802B", fontSize: "0.82rem", letterSpacing: "0.04em", fontWeight: 700, lineHeight: 1.6 }}
              >
                &gt; hover a node or click multiple nodes to load program data_
              </motion.p>
            )}
          </AnimatePresence>
        </motion.div>
