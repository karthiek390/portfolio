...
export default function RedCertCard({ name, issuer, year, certUrl }: RedCertCardProps) {
  const [state, setState] = useState<"idle" | "loading" | "loaded">("idle");
  const hasCertificate = Boolean(certUrl);

  const handleClick = () => {
    if (!hasCertificate || state !== "idle") return;
  ...
  return (
    <div
      ...
      style={{
        ...
        cursor: hasCertificate && state === "idle" ? "pointer" : "default",
      ...
      <AnimatePresence mode="wait">
        ...
        {state === "loading" && (
          <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <p style={{ color: "#00802B", fontSize: "0.72rem", marginBottom: "0.75rem" }}>LOADING: {name}...</p>
